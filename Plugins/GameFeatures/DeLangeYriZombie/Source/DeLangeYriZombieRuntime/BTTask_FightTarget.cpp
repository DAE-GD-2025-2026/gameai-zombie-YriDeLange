...
UBTTask_FightTarget::UBTTask_FightTarget()
{
	NodeName = "Fight Target";
	bNotifyTick = true;
	bCreateNodeInstance = true;
	TargetEnemyKey.AddObjectFilter(this, GET_MEMBER_NAME_CHECKED(UBTTask_FightTarget, TargetEnemyKey), ABaseZombie::StaticClass());
}

EBTNodeResult::Type UBTTask_FightTarget::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	return EBTNodeResult::InProgress;
}

void UBTTask_FightTarget::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
